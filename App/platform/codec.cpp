...
constexpr uint8_t to_codec_volume(uint8_t v) {
  return v > 100 ? 63 : static_cast<uint8_t>((static_cast<uint32_t>(v) * 63) / 100);
}
...
int32_t i2c4_write(uint16_t addr, uint16_t reg, uint8_t* data, uint16_t len) {
  return HAL_I2C_Mem_Write(&g_i2c4, addr, reg, I2C_MEMADD_SIZE_16BIT, data, len,
                           1000) == HAL_OK
             ? 0
             : -1;
}
...
int32_t i2c4_read(uint16_t addr, uint16_t reg, uint8_t* data, uint16_t len) {
  return HAL_I2C_Mem_Read(&g_i2c4, addr, reg, I2C_MEMADD_SIZE_16BIT, data, len,
                          1000) == HAL_OK
             ? 0
             : -1;
}
