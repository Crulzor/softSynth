...
int main(void)
{
  ...
  MX_GPIO_Init();
  /* NOTE (blue-screen milestone): the remaining MX_*_Init() calls from the
   * full board template are intentionally disabled for now. Some of them would
   * trap in Error_Handler() with nothing connected (e.g. MX_SDMMC1_MMC_Init
   * probes an eMMC that isn't present), and the display is brought up by the
   * C++ app with corrected timings/clock instead of the generic MX_LTDC_Init.
   * Re-enable peripherals here one at a time as the synth needs them (SAI2 for
   * audio, FMC for SDRAM framebuffers, etc.). */
#if 0
  MX_ADC1_Init();
  MX_ADC2_Init();
  MX_ADC3_Init();
